...
async function mcsrvstat(p: Parameters): Promise<response> {
	if (p.args.length !== 1) {
		return {
			color: red,
	...
	let status: any = await helper.download(
		`https://api.mcsrvstat.us/2/${p.args[0]}`
	);
	...
	if (status === "error") {
		...
		return {
			...
			message: `${p.args[0]} is unreachable by the MCSrvStat API`,
		};
	} else if (!Object.keys(status).includes("players")) {
		return {
			color: green,
			title: "Failed to get info",
			message: `${p.args[0]} is not a Minecraft server`,
		};
	}

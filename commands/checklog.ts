...
async function checklog(args: string[], authorID: string, author: string, channelID: string, channel: any, creator: any, bot: any){
    ...
    if(link.includes("https://pastebin.com")){
        ...
        return ({"color": red, "title": `Missing link`, "message": `**Missing URL.**`});
    }
    ...
    if(contents === "error"){
        return ({"color": red, "title": `Failed to download contents`, "message": `**Failed to download contents of link. Try again.**`});
    }

    // Process the contents if it's from ubuntu pastebin
    if(link.includes("https://paste.ubuntu.com") || link.includes("https://pastebin.ubuntu.com")){
    ...
    return({"color": green, "title": `Done scanning ${link} - ${type}`, "message": `${message}`});
}
